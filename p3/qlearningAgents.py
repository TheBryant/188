    ...
    def computeActionFromQValues(self, state):
        ...
        if len(Actions) == 0:
          ...
          for action in Actions:
            ...
            values.append(value)
          ranActions = [x for x in Actions if actionValues[x] == max(values)]
          return random.choice(ranActions)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        f = [features[x] for x in features]
        w = [self.weights[x] for x in self.weights]
        #print(features)
        #print (features)
        #print (self.weights)
        #print(sum(x*y for x in f for y in w))
        return sum(features[x]*self.weights[x] for x in features)
