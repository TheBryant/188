...
void Tracer_t::start()
{
   ...
   trace_data.two_issues  = 0;
   trace_data.mem_and_alu = 0;
}
...
*****/ 
   ...
   #define CHECK_ISSUE_SLOT(n) \
   if (tile->Top_BoomTile_core_dpath_issue_unit_IntegerIssueSlot_##n##__slot_valid.lo_word() && \
       tile->Top__io_debug_0_issue_slot_request_##n.lo_word()) \
   {  \
        num_ready_to_issue++; \
        if (tile->Top_BoomTile_core_dpath_issue_unit_IntegerIssueSlot_##n##__slotUop_is_store.lo_word() || \
            tile->Top_BoomTile_core_dpath_issue_unit_IntegerIssueSlot_##n##__slotUop_is_load.lo_word()) \
        { \
          has_mem = true; \
        } else { \
          has_alu = true; \
   ...
   // insert code that increments your counters
     if (num_ready_to_issue > 1){
      trace_data.two_issues++;
      if(has_mem && has_alu){
        trace_data.mem_and_alu++;
      }
     }
...
/* 
  ...
  */
   ...
   fprintf(logfile, "#        - Multi-Issues     : %lu\n", trace_data.two_issues);
   fprintf(logfile, "#        - Mem+ALU     : %lu\n", trace_data.mem_and_alu);
