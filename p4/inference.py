...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        posPos = len(self.legalPositions)

        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i%posPos])
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        if noisyDistance == None:
            self.particles = [self.getJailPosition() for x in range(self.numParticles)]
        else:
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] += emissionModel[trueDistance]# * (float(self.particles.count(p)) / self.numParticles)
    ...
    def getBeliefDistribution(self):
        ...
        for x in self.particles:
            belief[x] += (1/float(self.numParticles))
        return belief
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        jails = [self.getJailPosition(i) for i in range(self.numGhosts)]
        self.possiblePositions = list(itertools.product(self.legalPositions+jails, repeat=self.numGhosts))

        self.possibleParticles = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(self.possibleParticles)
        newParticles = [self.possibleParticles[i%len(self.possibleParticles)] for i in range(self.numParticles)]
        self.particles = newParticles
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] == None:
                for p in self.particles:
                    self.particles.append(self.getParticleWithGhostInJail(self.particles.pop(0), i))
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] == None:
                    ...
                    weight = float(weight) * float(emissionModels[i][trueDistance])
    ...
    def getBeliefDistribution(self):
        ...
        belief = util.Counter()
        for p in self.particles:
            belief[p] += (1/float(self.numParticles))
        return belief
