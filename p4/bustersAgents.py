...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i,alive in enumerate(livingGhosts):
            if alive == True:
                #lGPD = livingGhostPositionDistributions[i-1]
                ghostDistances[i] = self.ghostBeliefs[i-1].argMax()
        #print(ghostDistances)
        #closestGhost = min(ghostDistances, key=manhattanDistance())
        values = [x[1] for x in ghostDistances.items()]
        closestGhost = [x for x,y in ghostDistances.items() if ghostDistances[x] == max(values)][0]
        #print(ghostDistances)
        #print(closestGhost)
        targetloc = ghostDistances[closestGhost]

        actDis = {}
        for a in legal:
            actDis[a] = Actions.getSuccessor(pacmanPosition, a)
            #print actDis
        #print min(actDis, key=lambda x: self.distancer.getDistance(actDis[x], ghostDistances[closestGhost-1]))
        #print [self.distancer.getDistance(actDis[x], targetloc) for x in actDis]
        return min(actDis, key=lambda x: self.distancer.getDistance(actDis[x], targetloc))
