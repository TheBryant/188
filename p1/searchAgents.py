...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        print(self.startingPosition, self.found_corners)
        return (self.startingPosition, self.found_corners)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    #try distance to nearest pellet + # of pellets left
    distances = []
    pelletsLeft = len(foodGrid.asList())
    for pel in foodGrid.asList():
        x, y = pel
        #distances.append(abs(position[0] - x) + abs(position[1] - y))
        distances.append(((position[0] - x) ** 2 + (position[1] - y) **2 )** 0.5)

    if not distances == []:
        closestPel = foodGrid.asList()[distances.index(min(distances))] #hope this works
        prob = PositionSearchProblem(problem.startingGameState, start=position, goal=closestPel, warn=False, visualize=False)
        distanceClosest = len(search.bfs(prob))
    else:
        distanceClosest = 0

    #return (pelletsLeft*.5+distanceClosest*.5)
        ...
        while not fringe.isEmpty():
            ...
            if not(currentNode[0] in closed):
                ...
                for node in problem.getSuccessors(currentNode[0]):
                    ...
                    fringe.push((state, currentNode[1]+[action]))
